...
:
if. 0=read '' do. i. 0 0 return. end.
'DETSPACE DETEXPAND'=: opt=. detailoption x
y=. getnameloc y
sname=. getshortname 2 pick y
pn=. 'PM',(x pick TIMETEXT;SPACETEXT),' - ',sname
...
dat=. opt showtotal1 y
ftr=. +/ ('[rest]';'[total]') e. _2 {. 0 pick dat
dat=. (> each 2 {. dat), 2 }. dat
dataraw=: |: <"1 &> ,.each ((-ftr)&}.) each dat
dat=. x showtotalfmt dat
data=: |: <"1 &> dat
