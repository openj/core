...
int toascn(WCHAR* src, LPSTR snk, int snklen)
{
	LPSTR p;
...
STDMETHODIMP CJServer::Do(BSTR input, long *pr)
{
	...
	*pr = toascn(input, line, sizeof line);
...
STDMETHODIMP CJServer::GetM(BSTR name, long* jtype, long* jrank, long* jshape, long* jdata, long *pr)
{
	...
	*pr = toascn(name, aname, sizeof aname);
	if(*pr) return NOERROR;
...
STDAPI DllUnregisterServer(void)
{
	...
	while(p>keys)
	{
		p=strrchr(keys, '\t');	
		*p=0;
	    int r=RegDeleteKey(HKEY_CLASSES_ROOT, p+1);
